...
def bitmap_to_bitmapimage(bitmap):
    """ Converts a System.Drawing.Bitmap to a BitmapImage for use with WPF"""
    ocs = MemoryStream()
    bitmap.Save(ocs, ImageFormat.Png)
    ocs.Position = 0
    bitimg = BitmapImage()
    bitimg.BeginInit()
    bitimg.StreamSource = ocs
    bitimg.EndInit()
    return bitimg
